...
namespace data_assimilation::internal {
    ...
    Matrix2D<double> bin(
        ...
        const string& type = "linear"
    );

    data_assimilation::ObservationSpace convertToObservationSpace(const Matrix2D<double>& data);

    void getParameterBool(ParametersIni& parameters, const string& name, bool& parameter, bool required);

    data_assimilation::ProcessedMatFileData readData(double timeStart, double timeEnd, const pmf::Parameters& parameters);

    data_assimilation::ProcessedMatFileData cat(const vector<ProcessedMatFileData>& pmfDataSplit);
   ...
   vector<vector<data_assimilation::Observations>> interpolate_old(
        const data_assimilation::ProcessedMatFileData& data,
        const Matrix2D<double>& V_grid,
        const Matrix2D<double>& K_grid 
    );

   vector<vector<data_assimilation::Observations>> interpolate(

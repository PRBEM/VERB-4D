    ...
    for (int it = 0; it < num_steps; it++) {
        ...
        for (ix = 0; ix <= x_size - 1; ix++) {
            ...
            if (always_1st_order) {
                ...
                PSD_f[ix] = PSD_f[ix] + 0.5 * fabs(CourNum_f[ix]) / CourNum_f[ix] * d1;
            } else {
                PSD_f[ix] = 0.5 * (PSD_C + PSD_D[1]);  // A.1
                ...
                if (always_3rd_order) {  // !never_3rd_order && (always_3rd_order || (CURVAV < THC1 && GRAD < THG))) {
                    ...
                    PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) / (3 * 2 * 1) * (D2 + d3 / 2);  // A.16

                } else {
                    // 4 order - PSD_f is still second order
                    PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) / (3 * 2 * 1) * (D2 + fabs(CourNum_f[ix]) / 4 * d3);
                    ...
                    if (always_5rd_order) {  // !never_5rd_order && always_5rd_order) {
                        // 5 order
                        PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) * (pow(CourNum_f[ix], 2) - pow(2., 2.)) / (5 * 4 * 3 * 2 * 1) * (D4 + d5 / 2);

                    } else {
                        // 6 order - PSD_f is still 4th order.
                        PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) * (pow(CourNum_f[ix], 2) - pow(2., 2.)) / (5 * 4 * 3 * 2 * 1) * (D4 + fabs(CourNum_f[ix]) / 6 * d5);
                        ...
                        if (always_7rd_order) {  // !never_7rd_order && (always_7rd_order || (CURVAV < THC2 && GRAD < THG))) {
                            // 7 order
                            PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) * (pow(CourNum_f[ix], 2) - pow(2., 2.)) * (pow(CourNum_f[ix], 2) - pow(3., 2.)) / (7 * 6 * 5 * 4 * 3 * 2 * 1) * (D6 + d7 / 2);
                        } else {
                            // I think there is a bug somewhere here for 8-9 orders :( <- this comment is not from hayley
                            // 8 order
                            PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) * (pow(CourNum_f[ix], 2) - pow(2., 2.)) * (pow(CourNum_f[ix], 2) - pow(3., 2.)) / (7 * 6 * 5 * 4 * 3 * 2 * 1) * (D6 + fabs(CourNum_f[ix]) / 8 * d7);
                            ...
                            // 9 order
                            PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f[ix], 2) - pow(1., 2.)) * (pow(CourNum_f[ix], 2) - pow(2., 2.)) * (pow(CourNum_f[ix], 2) - pow(3., 2.)) * (pow(CourNum_f[ix], 2) - pow(4., 2.)) / (9 * 8 * 7 * 6 * 5 * 4 * 3 * 2 * 1) * (D8 + d9 / 2);
                        }
                    }
                }
            }
            ...
            if (use_limiting && (!use_discreminator || !(  // if don't use the descriminator - always go inside
                                                           ...
                                                           (((dif1 > treshhold && dif2 > treshhold) && (dif3 < -treshhold && dif4 < -treshhold)) && ((fabs(dif2) < fabs(dif1) - treshhold) && (fabs(dif3) < fabs(dif4) - treshhold))) || (((dif2 > treshhold && dif3 > treshhold) && (dif4 < -treshhold && dif5 < -treshhold)) && ((fabs(dif3) < fabs(dif2) - treshhold) && (fabs(dif4) < fabs(dif5) - treshhold))) || (((dif3 > treshhold && dif4 > treshhold) && (dif5 < -treshhold && dif6 < -treshhold)) && ((fabs(dif4) < fabs(dif3) - treshhold) && (fabs(dif5) < fabs(dif6) - treshhold)))))) {
                //			if (!(
                ...
                if (ACURV >= ADEL) {  // (4)
                    ...
                    PSD_f[ix] = PSD_C;
                } else if (fabs(CourNum_f[ix]) > 1e-31) {  // this will not work if velocity is zero
                    // cont. from page 48

			...
			if (true) {
				...
				if (always_3rd_order) { // !never_3rd_order && (always_3rd_order || (CURVAV < THC1 && GRAD < THG))) {
					...
					if (always_5rd_order) { // !never_5rd_order && always_5rd_order) {
						// 5 order
						PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f,2) - pow(1.,2.)) * (pow(CourNum_f,2) - pow(2.,2.))/(5*4*3*2*1)
														* (D4 + d5 / 2);

					} else {
						// 6 order
						PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f,2) - pow(1.,2.)) * (pow(CourNum_f,2) - pow(2.,2.))/(5*4*3*2*1)
														* (D4 + fabs(CourNum_f)/6 * d5);
						...
						if (always_7rd_order) { // !never_7rd_order && (always_7rd_order || (CURVAV < THC2 && GRAD < THG))) {
							// 7 order
							PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f,2) - pow(1.,2.)) * (pow(CourNum_f,2) - pow(2.,2.)) * (pow(CourNum_f,2) - pow(3.,2.))/(7*6*5*4*3*2*1)
															* (D6 + d7 / 2);
						} else {
							// I think there is a bug somewhere here for 8-9 orders :(
							// 8 order
							PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f,2) - pow(1.,2.)) * (pow(CourNum_f,2) - pow(2.,2.)) * (pow(CourNum_f,2) - pow(3.,2.))/(7*6*5*4*3*2*1)
															* (D6 + fabs(CourNum_f)/8 * d7);
							...
							// 9 order
							PSD_f[ix] = PSD_f[ix] + (pow(CourNum_f,2) - pow(1.,2.)) * (pow(CourNum_f,2) - pow(2.,2.)) * (pow(CourNum_f,2) - pow(3.,2.)) * (pow(CourNum_f,2) - pow(4.,2.))/(9*8*7*6*5*4*3*2*1)
															* (D8 + d9 / 2);
						}
					}
				}
...
//				)) {
				...
				if (fabs(CourNum_f) > 1e-31	) { // this will not work if velocity is zero
					// cont. from page 48
					// !!! PSD_REF = PSD_U + (PSD_C - PSD_U)/CourNum[ix]; // (3)
					// !!! PSD_REF = PSD_U + (PSD_C - PSD_U)/CourNum_f; // (3)
					PSD_REF = PSD_U[1] + (PSD_C - PSD_U[1]) / fabs(CourNum_f); // (3)
		...
		// update PSD
		double PSD_r, PSD_l; // values on left and right faces (between (i and i-1), and (i and i+1))
		for (ix = 0; ix < x_size-1; ix++) {
			...
			PSD[ix] = PSD[ix] - CourNum[ix] * (PSD_r - PSD_l);
		}

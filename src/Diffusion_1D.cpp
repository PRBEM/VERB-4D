		...
		double dt) 
{
	std::tuple<CalculationMatrix, CalculationMatrix, CalculationMatrix> matrices = 
		getCalculationMatrix1DDiffusion(
			x, x_size,
			x_LBC, x_UBC,
	...
	CalculationMatrix matr_A = std::get<0>(matrices);
	CalculationMatrix matr_B = std::get<1>(matrices);
	CalculationMatrix matr_C = std::get<2>(matrices);

	// make RHS = B*f + C
	DiagMatrix::iterator it_B;
	Matrix1D<double> RHS(matr_A.total_size);
	for (int ix = 0; ix < x_size; ix++) {
		int in = matr_B.index1d(ix);
		RHS[in] = matr_C[0][in];
...
std::tuple<CalculationMatrix, CalculationMatrix, CalculationMatrix> getCalculationMatrix1DDiffusion(
	...
	return std::make_tuple(matr_A, matr_B, matr_C);
